...
function init3DObjects(sceneGraph) {
    ...
    const sphereGeometry = primitiveGeometry.getSphereGeometry(
        new THREE.Vector3(0, 1.0, 1.25), 0.5);
...
function animate(sceneThreeJs, time) {
    ...
    // Rotation around the center of the object attached to a static object
    const Tinv_sphere = new THREE.Matrix4().makeTranslation(0,-1,-1.25);
    const R_sphere = new THREE.Matrix4()
        .makeRotationAxis(new Vector3(0.0, 0.0, 1.0), -0.01);
